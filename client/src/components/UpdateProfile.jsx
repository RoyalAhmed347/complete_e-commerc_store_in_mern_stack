...
const UpdateProfile = () => {
  const { logInUser, token, setLSUser, getSingelUser } = useAuthHook();
  ...
  useEffect(() => {
    const { firstName, lastName, phone, email } = logInUser;
    setUser({
      firstName,
      lastName,
      phone,
  ...
  const updateProfile = async (e) => {
    ...
    try {
      ...
      const result = await axios.patch(
        "http://localhost:3002/api/auth/user/profileImg",
        formData,
        {
          headers: {
  ...
  const submitHeandller = async (e) => {
    ...
    try {
      ...
      setLSUser(result.data.updatedUser);
                  ...
                  <label htmlFor="img_upload">
                    <img
                      src={
                        image.preview
                          ? image.preview
                          : `http://localhost:3002/${logInUser.avtarURL}`
                      }
